...
import com.tyyd.common.codebook.PayDItemEnum;
...
@Service("assetPayImportService")
public class AssetPayImportServiceImpl extends ExcelImport2BaseService {
	
	private ThreadLocal<Map<String, Long>> assetIdLocalMap = new ThreadLocal<Map<String, Long>>();

	@Override
	public String getTempletePath() {
		return "classpath:template/assetImportPayDetailParseRule.xlsx";
	}
	...
	public Map<String, Object> doSaveData(MultipartHttpServletRequest request, HttpServletResponse reponse,
		...
		List<List<AssetPayDetailDto>> saveList = newNoerrList.stream().map(bean -> {
			Map<String, Long> assetMap = assetIdLocalMap.get();
			Long assetId = assetMap.get(StringUtils.trim(bean.getValue("ASSET_CD")));
			List<AssetPayDetailDto> dtoList = new ArrayList<AssetPayDetailDto>();
			dtoList.add(convet2AssetPayDetailDto(assetId, StringUtils.trim(bean.getValue("REMUNERATION")),
					PayDItemEnum.REMUNERATION.getCd(), contractCd));
			dtoList.add(convet2AssetPayDetailDto(assetId, StringUtils.trim(bean.getValue("VALUE_ADDED_TAX")),
					PayDItemEnum.VALUE_ADDED_TAX.getCd(), contractCd));
			dtoList.add(convet2AssetPayDetailDto(assetId, StringUtils.trim(bean.getValue("ADDITIONAL_TAX")),
					PayDItemEnum.ADDITIONAL_TAX.getCd(), contractCd));
			dtoList.add(convet2AssetPayDetailDto(assetId, StringUtils.trim(bean.getValue("INCOME_TAX")),
					PayDItemEnum.INCOME_TAX.getCd(), contractCd));
			dtoList.add(convet2AssetPayDetailDto(assetId, StringUtils.trim(bean.getValue("OTHER")),
					PayDItemEnum.OTHER.getCd(), contractCd));
			return dtoList;
		}).collect(Collectors.toList());
		List<AssetPayDetailDto> dataList = saveList.stream().flatMap(dtoList -> dtoList.stream().map(dto -> dto))
				.collect(Collectors.toList());
		// 2.执行导入操作
		...
		scfAssetValueEditService.batchSavePayDetail(Security.getCurrentUserID(), dataList,
				contractCd);
	 ...
	 * @return
	 */
	private AssetPayDetailDto convet2AssetPayDetailDto(Long assetId, String money, String cd, String contractCd) {
		AssetPayDetailDto dto = new AssetPayDetailDto();
